...
QueueFamilyIndex findQueueFamilies(VkPhysicalDevice physicalDevice_);
    ...
    for (const char* layer_name : validation_layers) {
        for (const VkLayerProperties& layer_properties : available_layers) {
            if (strcmp(layer_name, layer_properties.layerName) == 0) {
                ...
                minilog::log_info("the {} is supported!", layer_name);
                break;
            }
        }
        if (validation_layer_supported) { break; }
    }
...
void create_instance() {
    // pNext is a 'const void*' type,
    //   so it can point to an arbitrary structure,
    ...
    if (enable_validation_layers) {
        instanceCreateInfo.pNext = (VkDebugUtilsMessengerCreateInfoEXT*) &debug_create_info;
...
void createLogicalDevice() {
    ...
    vkGetDeviceQueue(logicalDevice, indices.presents.value(), 0, &presentQueue);
}
...
void createGraphicsPipeline() {
    std::vector<char> vertCode = readFile("./src/2_triangle/shaders/vert.spv");
    std::vector<char> fragCode = readFile("./src/2_triangle/shaders/frag.spv");
...
SwapChainSupportDetails querySwapChainSupport(VkPhysicalDevice physicalDevice_) {
    ...
    vkGetPhysicalDeviceSurfacePresentModesKHR(physicalDevice_, surface, &presentModeCount, details.present_modes.data());
